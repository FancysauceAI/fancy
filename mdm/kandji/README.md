...
`FancysauceAI/fancy`. Nothing is bundled here.
...
the user runs `claude plugin install fancysauce-savings-preview@fancy` once. Read
...
fancysauce-savings-preview@fancy` once. `CC_TAG`/`CC_SHA` are read only in
...
- `managed-hooks` (the default) — a drop-in fragment:
  ...
  `"enabledPlugins": { "fancysauce-savings-preview@fancy": false }`. There is **no**
