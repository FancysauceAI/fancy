...
REPO="https://github.com/FancysauceAI/fancy.git"
PIN_FILE=""
case "$TOOL" in
  codex)
    CACHE="$HOME/.cache/fancysauce/codex"
    DATA="$HOME/.local/share/fancysauce/codex/data"
    COLLECT_REL="plugins/fancysauce-savings-preview/dist/agents/codex/collect.mjs"
    ;;
  claude-code)
...
if [ "$CC_MODE" = "managed-hooks" ]; then
  ...
  CC_EVENTS="SessionStart SessionEnd UserPromptSubmit PreToolUse PostToolUse PostToolUseFailure SubagentStart SubagentStop PreCompact PostCompact Stop StopFailure ConfigChange Notification TaskCompleted PermissionRequest"
  {
    ...
    /usr/bin/printf '  "enabledPlugins": { "fancysauce-savings-preview@fancy": false }\n}\n'
