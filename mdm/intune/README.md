...
The plugin (`fancysauce-savings`) comes from the GitHub dist repo `FancysauceAI/fancy`; it is not bundled in this package. Managed settings register the marketplace and enable the plugin, but do not install it — each user runs `claude plugin install fancysauce-savings-preview@fancy` once.
