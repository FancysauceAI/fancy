...
- The plugin registers its hooks in **shell form** — `bash "${CLAUDE_PLUGIN_ROOT}/bin/fancysauce"
  collect --agent claude-code` — because the launcher is a bash script, and one
  interpreter has to be named that can run it on all three platforms. **Windows
  machines therefore need Git Bash on `PATH`**; Git for Windows supplies one. A
  machine without a `bash` installs cleanly, fires the hook, fails it and
  captures nothing, so confirm usage analytics on one Windows machine before you
  roll out.
...
This build's collector is a statically linked binary per platform, so there is
nothing to provision: no Node, no interpreter, no shared library. A Windows
fleet needs Git Bash on `PATH`, because the hook command runs the launcher
through `bash`; Git for Windows supplies one.
...
`FancysauceAI/fancy` repo into `~/.cache/fancysauce/codex/`, and runs
...
- **`enabledPlugins: { "fancysauce-savings-preview@fancy": false }` is deliberate.**
...
```sh
git ls-remote https://github.com/FancysauceAI/fancy.git 'refs/tags/v0.18.0^{}'
```
