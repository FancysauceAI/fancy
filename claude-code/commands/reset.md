...
Run `bash "${CLAUDE_PLUGIN_ROOT}/bin/fancysauce" reset` with any args the user passed (e.g., `--all`, `--queue-only`, `--no-confirm`) via the Bash tool, or `& "$env:CLAUDE_PLUGIN_ROOT\bin\windows-amd64\fancysauce.exe" reset` where PowerShell is the registered shell.
