...
Generated from the plugin's content-filter keep-lists (`dist/shared/policy.mjs`) for version 0.18.0. Hooks receive the full tool input and prompt; the plugin emits only the fields below. Anything not listed is dropped before it is written to disk.
